...
const Navigation = () => {
  ...
  return (
    <nav className={cn(
      ...
      isScrolled ? "h-16 bg-background/80 backdrop-blur-sm shadow-md" : "h-20 bg-transparent"
    )}>
      <div className="container mx-auto h-full flex items-center justify-between px-4">
        <div className="flex items-center gap-2">
            <Car className="text-foreground" size={isScrolled ? 24 : 28} />
            <span className={cn("font-headline text-xl md:text-2xl tracking-widest", isScrolled ? "text-xl": "text-2xl")}>ScrollMotion</span>
        </div>
        ...
        <div className="flex items-center gap-2">
            <SearchModal />
            <Button size={isScrolled ? "sm" : "default"} className="font-body font-semibold uppercase tracking-widest text-xs">Get Quote</Button>
        </div>
      </div>
    </nav>
  );
