...
class Geometry {
  ...
  spherize() {
    this.vertices.forEach(vertex => {
      ...
      vec3.scale(vertex.position, vertex.normal, 1);
    });
    return this;
  }
...
function createProgram(gl: WebGL2RenderingContext, shaderSources: [string, string], transformFeedbackVaryings: string[] | null, attribLocations: { [key: string]: number } | null): WebGLProgram | null {
...
class InfiniteGridMenu {
  ...
  canvas: HTMLCanvasElement;
  items: InfiniteGridMenuItem[];
  onActiveItemChange: (index: number) => void;
  onMovementChange: (isMoving: boolean) => void;

  constructor(canvas: HTMLCanvasElement, items: InfiniteGridMenuItem[], onActiveItemChange: (index: number) => void, onMovementChange: (isMoving: boolean) => void, onInit: ((sketch: InfiniteGridMenu) => void) | null = null, scale = 1.0) {
    this.canvas = canvas;
    this.items = items || [];
  ...
  #init(onInit: ((sketch: InfiniteGridMenu) => void) | null) {
    ...
    if (!discProgram) throw new Error("Failed to create disc program");
    ...
    if (!discVAO) throw new Error("Failed to create disc VAO");
    this.discVAO = discVAO;

    this.icoGeo = new IcosahedronGeometry();
    this.icoGeo.subdivide(1).spherize();
  ...
  #findNearestVertexIndex(): number | null {
...
export default function InfiniteMenu({ items = [], scale = 1.0 }: { items: Item[], scale?: number}) {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // Here you might want to clean up the sketch, like stopping the animation loop
    };
  }, [items, scale]);
